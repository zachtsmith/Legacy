...
export default function ApplicationViews({ isLoggedIn, isBroker, user }) {
  return (
    <main  >
      ...
      <Routes>
        <Route path="/">
          ...
          <Route path="carrier" element={isLoggedIn ? <CarrierList isBroker={isBroker} user={user} /> : <Navigate to="/login" />} />
          <Route path="carrier/create" element={isLoggedIn ? <CarrierForm user={user}/> : <Navigate to="/login" />} />
